...
export class ShopeeCrawler extends PuppeteerCrawlerBase {
     ...
     */
    private parseNetscapeCookies(content: string): any[] {
        ...
        for (const line of lines) {
            ...
            cookies.push({
                ...
                path: path || '/',
                expires: parseInt(expiration) || (Date.now() / 1000 + 86400 * 30), // Default 30 days if 0
                secure: secure === 'TRUE',
                httpOnly: false, // Netscape format doesn't explicitly flag httpOnly
            });
        }
        ...
        try {
            ...
            if (this.cookies) {
                ...
                if (this.cookies.startsWith('[')) {
                    cookieArray = JSON.parse(this.cookies);
                } else if (this.cookies.includes('\t')) {
                    cookieArray = this.parseNetscapeCookies(this.cookies);
                } else {
                    // Simple string format, try to extract csrftoken manually if needed
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            const crawlPage = async (): Promise<CrawledProduct[]> => {
                ...
                try {
                    ...
                    if (this.cookies) {
                        try {
                            const trimmedCookies = this.cookies.trim();
                            if (trimmedCookies.startsWith('[')) {
                                const rawCookies = JSON.parse(trimmedCookies);
                                cookieObjects = rawCookies;
                            } else if (trimmedCookies.includes('\t')) {
                                cookieObjects = this.parseNetscapeCookies(trimmedCookies);
                            } else {
                                // Simple string fallback
                                cookieObjects = trimmedCookies.split(';').map(pair => {
                                    const parts = pair.split('=');
                                    if (parts.length < 2) return null;
                                    return { name: parts.shift()?.trim(), value: parts.join('=').trim(), domain: '.shopee.vn', path: '/' };
                                }).filter((c): c is any => c !== null);
                            }
                        } catch (e) {
                            logger.error(`[Shopee] Failed to parse option cookies:`, e);
                        }
                    }
    ...
    async massCrawl(options: { pagesPerCategory?: number } = {}): Promise<CrawledProduct[]> {
        ...
        try {
            ...
            logger.info(`🎉 [Shopee] MASS CRAWL COMPLETE: ${totalSaved} products, ${totalErrors} errors`);
        } finally {
            await this.releaseBrowser();
        }
