...
import { VietnameseTokenizer } from './tokenizer';
import { BM25Ranker } from './ranking';
...
export class SearchService {
    ...
    private async fullTextSearch(
        ...
        // Apply BM25 Re-ranking to top candidates if we have enough results
        if (results.length > 5) {
            const ranker = new BM25Ranker();
            ranker.train(results.map(r => ({ id: r.id, text: r.name })));

            results.forEach(r => {
                const bm25Score = ranker.score(query, { id: r.id, text: r.name });
                r.relevanceScore = (r.relevanceScore || 0) + bm25Score;
                r.matchType = 'exact';
            });

            // Sort by combined score if not strictly searching by date/price
    ...
    private normalizeQuery(query: string): string {
        // Use the specialized Vietnamese Tokenizer
        return VietnameseTokenizer.forSearch(query);
    }
