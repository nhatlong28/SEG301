/**
 * Debug Dienmayxanh Crawler
 * 
 * Crawl theo categories thay vì keywords
 * Usage: npx tsx src/scripts/debug_dmx.ts
 */
...
const DATA_DIR = path.join(process.cwd(), 'data', 'dienmayxanh');
...
// DMX Categories với nhiều sản phẩm
const CATEGORIES = [
    '/tivi', '/tu-lanh', '/may-giat', '/may-lanh', '/may-loc-nuoc',
    '/loa', '/tai-nghe', '/dong-ho-thong-minh', '/may-tinh-bang',
    '/may-hut-bui', '/noi-com-dien', '/noi-chien-khong-dau', '/lo-vi-song',
    '/bep-dien', '/quat-dieu-hoa', '/may-say-toc', '/ban-la', '/may-xay-sinh-to'
];

async function main() {
    console.log('╔═══════════════════════════════════════════════════════╗');
    console.log('║        DIENMAYXANH DEBUG CRAWLER                      ║');
    console.log('╚═══════════════════════════════════════════════════════╝\n');
    ...
    for (const category of CATEGORIES) {
        const url = `https://www.dienmayxanh.com${category}`;
        console.log(`\n📌 Category: ${category}`);
                ...
                const products = await page.evaluate(() => {
                    ...
                    document.querySelectorAll('.listproduct li').forEach((li: any) => {
                        ...
                        if (name && href) {
                            items.push({
                                externalId: href,
                                externalUrl: `https://www.dienmayxanh.com${href}`,
                                name, price, imageUrl
                            });
                        }
                    });
